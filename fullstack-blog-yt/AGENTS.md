...
| その他 | `BarcodeDetector API` | — | `/delivery`(納品検品スキャン) / `/shipment`(出庫検品スキャン) で QR 読取 |
...
│  │  └─ edit/[id]/page.tsx   # 編集
│  ├─ delivery/
│  │  ├─ page.tsx             # 納品検品スキャン(納品先選択→検品)
│  │  └─ DeliveryInspection.tsx
│  ├─ shipment/
│  │  ├─ page.tsx             # 出庫検品スキャン(複数 QR 同時)
│  │  └─ ShipmentInspection.tsx
...
│  │  └─ barcode-detector.ts  # BarcodeDetector API の型(delivery/shipment 共用)
...
- クライアント側専用コンポーネントは `'use client'` を付け、ページ直下に同居(例: [app/delivery/DeliveryInspection.tsx](app/delivery/DeliveryInspection.tsx))
